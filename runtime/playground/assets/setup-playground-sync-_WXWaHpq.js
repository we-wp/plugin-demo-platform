import{ak as c,al as p,l as u}from"./opfs-site-storage-DRy7robp.js";import{h as m,n as y}from"./fs-journal-DLa4QJaK.js";const h=`<?php
...
playground_sync_start();`;async function f(e){await e.fileExists("/wordpress/wp-content/mu-plugins")||await e.mkdir("/wordpress/wp-content/mu-plugins"),await e.writeFile("/wordpress/wp-content/mu-plugins/sync-mu-plugin.php",h)}async function g(e,n,t={}){const r=await e.run({code:`<?php
        require '/wordpress/wp-load.php';
        playground_sync_override_autoincrement_algorithm(
			${c(n)},
			${c(t)}
		);
	    `});return d(r,"Initialization failed."),(await e.run({code:`<?php
        require '/wordpress/wp-load.php';
		$data = $GLOBALS['@pdo']
			->query('SELECT * FROM playground_sequence')
			->fetchAll(PDO::FETCH_KEY_PAIR);
		echo json_encode($data);
		`})).json}async function E(e,n){let t=null;e.addEventListener("request.end",()=>{t=null}),e.onMessage(async r=>{const s=JSON.parse(r);if(s?.type==="sql"){if(s.subtype==="transaction"){const o=s;if(!o.success)return;switch(o.command){case"START TRANSACTION":t=[];break;case"COMMIT":t?.length&&t.forEach(n),t=null;break;case"ROLLBACK":t=null;break}return}if(s.subtype==="replay-query"||s.subtype==="reconstruct-insert"){const o=s;t?t.push(o):n(o)}}})}async function $(e,n){const t=p({journal:n}),r=await e.run({code:`<?php
	...
	`});d(r,"Replay error.")}function d(e,n){if(e.text.trim()||e.errors.trim())throw u.error({text:e.text,errors:e.errors}),new Error(`${n}. See the console for more details.`)}async function q(e,n){await e.journalFSEvents("/wordpress/wp-content",async t=>{t.path.endsWith("/.ht.sqlite")||t.path.endsWith("/.ht.sqlite-journal")||n(t)})}class A{sendChanges(n){window.top.postMessage({type:"playground-change",envelope:n},"*")}onChangesReceived(n){window.addEventListener("message",t=>{t.data.type==="playground-change"&&n(t.data.envelope)})}}class v{sendChanges(){}onChangesReceived(n){this.injectChanges=n}injectChanges(n){}}function N(e){return{beforeSend:n=>((n.sql.length>0||n.fs.length>0)&&u.log(`[${e}] Sending changes`,n),n),afterReceive:n=>(u.log(`[${e}] Received changes`,n),n)}}const w=(e,n="https://playground.wordpress.net")=>({beforeSend:_(e,n),afterReceive:_(n,e)});function _(e,n){e=e.replace(/\/$/,""),n=n.replace(/\/$/,"");const t=new RegExp(`(^|[^0-9a-zA-Z])(?:${T(e)})($|[^0-9a-zA-Z])`,"g"),r=`$1${n}$2`;return s=>({...s,sql:s.sql.map(o=>{if(o.subtype==="replay-query")return{...o,query:o.query.replace(e,n)};if(o.subtype==="reconstruct-insert"){const i={...o.row};for(const a in i)typeof i[a]=="string"&&(i[a]=i[a].replace(t,r));return{...o,row:i}}return o})})}function T(e){return e.replace(/[/\-\\^$*+?.()|[\]{}]/g,"\\$&")}const S=()=>({beforeSend:e=>({...e,sql:e.sql.filter(b)}),afterReceive:e=>e}),b=e=>{if(e.query_type==="SELECT")return!1;const n=e.query_type,t=e.table_name?.toLowerCase();if(e.subtype==="replay-query"){const r=e.query.trim();if(n==="UPDATE"&&t==="wp_options"&&r.endsWith("`option_name` = 'cron'"))return!1}if(e.subtype==="reconstruct-insert"){if(t==="wp_options"){const r=e.row.option_name+"";if(r.startsWith("_transient_")||r.startsWith("_site_transient_"))return!1}if(t==="wp_usermeta"&&e.row.meta_key==="session_tokens")return!1}return!0},L=e=>({beforeSend:async n=>({...n,fs:await m(e,y(n.fs))}),afterReceive:n=>n});async function O(e,{autoincrementOffset:n,transport:t,middlewares:r=[]}){r=[S(),w(await e.absoluteUrl),L(e),...r],await f(e),await g(e,n),t.onChangesReceived(async a=>{for(const l of r)a=await l.afterReceive(a);await e.replayFSJournal(a.fs),await $(e,a.sql)});let s={fs:[],sql:[]};E(e,a=>{s.sql.push(a)}),q(e,a=>{s.fs.push(a)});const o=async()=>{let a=s;s={fs:[],sql:[]};for(const l of r)a=await l.beforeSend(a);!a.sql.length&&!a.fs.length||t.sendChanges(a)},i=async(a,l)=>{await a(),setTimeout(i,l,a,l)};i(o,3e3)}export{v as N,A as P,N as l,O as s};
//# sourceMappingURL=setup-playground-sync-_WXWaHpq.js.map
